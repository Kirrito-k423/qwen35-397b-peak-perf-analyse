...
```text
A5/A3_card 性能比 = T3_card / T5 = 0.73
(1 + C3_die) / 2 / 3 = 0.73
1 + C3_die = 4.38
C3_die = 3.38
```
...
```text
C3_die / C5 = 3.38 / 2 = 1.69x
```

这比“理论 2x”更保守。换句话说，在当前配置和 profiler 口径下，A5 相对 A3 单 die 的有效计算优势约为 `1.69x`，不是 `2x`。

核心修正结论：

1. 若 A3/A5 的计算都减半，A5/A3 单卡性能比从 `0.73x` 变为约 `0.67x`，方向仍然是变差。
2. 若双方未掩盖通信都减半，A5/A3 单卡性能比约为 `0.78x`；若双方通信都消掉，在当前反推的 `1.69x` 实际计算比下，上限约为 `0.845x`。
3. 若主要只降低 A5 的有效未掩盖通信，A5 通信减半时约为 `0.876x`，A5 通信完全消掉时约为 `1.095x`。
...
5. 因此当前甜点配置的目标不是追求“接近 2x A3 单卡”，而是先把 `0.73x` 拉到 `0.85x-1.0x`；要稳定超过 A3 单卡，需要 A5 侧通信/等待显著低于 A3，或 A5 实际计算优势超过 `2x` 单 die 预估。
...
A3 单 die:
  M3_die = 1
  C3_die = 3.38  # 由 0.73x 实测反推
  T3_die = 1 + 3.38 = 4.38

A3 单卡两 die:
  T3_card = T3_die / 2 = 2.19

当前性能比:
  A5/A3_card = T3_card / T5 = 2.19 / 3 = 0.73
```

注意这个 `3.38` 是当前配置下的反推值。它不等于理论硬件峰值，只表示在当前训练、并行、profiling 口径下，A3 单 die 的计算耗时相对 A5 计算耗时约为 `1.69x`。
...
但实测是 `0.73`，所以当前实际有效计算优势低于预估，或者 A5 的未掩盖等待/同步被统计到了计算/通信关键路径里。
...
A3 单 die:
  T3_die_new = 1 + 3.38/2 = 2.69

A3 单卡:
  T3_card_new = 2.69 / 2 = 1.345
```

新的性能比：

```text
A5/A3_card_new = 1.345 / 2 = 0.6725
```
...
A3_card speedup = 2.19 / 1.345 = 1.628x
new_ratio = 0.73 * 1.50 / 1.628 ~= 0.67
```

结论：双方计算都减半时，A5/A3 单卡性能比会从 `0.73x` 降到约 `0.67x`。原因是 A3 单卡两 die 后仍然很吃计算优化，计算优化对 A3 单卡也很有利。
...
```text
A5_new = q + 2
A3_card_new = (q + 3.38) / 2
ratio_new = (q + 3.38) / [2 * (q + 2)]
```

| 双方通信缩放 q | A5 耗时 | A3 单卡等价耗时 | A5/A3 单卡 |
|---:|---:|---:|---:|
| `1.00` | `3.00` | `2.19` | `0.73x` |
| `0.75` | `2.75` | `2.065` | `0.751x` |
| `0.50` | `2.50` | `1.94` | `0.776x` |
| `0.25` | `2.25` | `1.815` | `0.807x` |
| `0.00` | `2.00` | `1.69` | `0.845x` |

结论：如果通信优化对 A3/A5 等比例生效，在当前反推的 `1.69x` 实际计算比下，即使双方通信都消掉，A5/A3 单卡也只有约 `0.845x`。

### 4.2 只降低 A5 的有效未掩盖通信

如果并行策略主要解决 A5 的通信域映射、MoE straggler、collective 等待，而 A3 单卡结果基本不变，则 A3 单卡等价耗时仍为 `2.19`：
...
|---:|---:|---:|---:|
| `1.00` | `3.00` | `1.00x` | `0.73x` |
| `0.75` | `2.75` | `1.09x` | `0.796x` |
| `0.50` | `2.50` | `1.20x` | `0.876x` |
| `0.25` | `2.25` | `1.33x` | `0.973x` |
| `0.00` | `2.00` | `1.50x` | `1.095x` |

结论：如果能只减少 A5 的未掩盖通信/等待，通信减半可到约 `0.876x`，通信全消可到约 `1.095x`。
...
```text
A5_new = q + 2p
A3_card_new = (q + 3.38p) / 2
ratio_new = (q + 3.38p) / [2 * (q + 2p)]
```
...
```text
p = q
ratio_new = (q + 3.38q) / [2 * (q + 2q)] = 4.38 / 6 = 0.73
```
...
这等价于 `p > 1, q ~= 1`。在当前反推的 `1.69x` 计算比下：
...
| `1.50` | `1.00` | `0.759x` |
| `2.00` | `1.00` | `0.776x` |
| `infinity` | `1.00` | `0.845x` |

因此，增加计算占比会改善 A5/A3 单卡比值，但在当前反推的有效计算优势 `1.69x` 下，上限仍约 `0.845x`。若 A5 实际计算优势能恢复到理论 `2x`，纯计算上限才约 `1.0x`。

### 7.3 总 token 数不变且 A5 MFU 随长序列提升

更长序列可能让 A5 的 GEMM/attention 更饱满，使实际 `C3_die/C5` 从 `1.69x` 往 `2x` 靠近。这种情况下序列长度调整有双重收益：

1. 通信量基本不变，计算占比提高。
2. A5 计算效率可能提升，使实际计算比从 `1.69x` 接近 `2x`。
...
- 当前 `0.73x` 反推得到 A5 实际计算优势约为 A3 单 die 的 `1.69x`。
- 双方计算都减半，A5/A3 单卡约 `0.67x`。
- 双方通信都减半，A5/A3 单卡约 `0.78x`。
- 只降低 A5 通信，通信减半约 `0.876x`，通信全消约 `1.095x`。
- 若 A5 有效计算优势恢复到理论 `2x`，通信不变约 `0.83x`；双方通信都消掉约 `1.0x`；只消 A5 通信可到约 `1.25x`。

所以甜点配置的目标应改成：

```text
先把 A5/A3_card 从 0.73x 拉到 0.85x-1.0x
优先减少 A5-only 的未掩盖通信和 MoE 长尾
验证 A5 实际计算优势能否从 1.69x 恢复到 2x
...
5. 用 `0.73x` 继续反推实际计算比，而不是直接套用理论 `2x`。
